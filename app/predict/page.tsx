...
export default function PredictPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const modelInput = Object.entries(formData).reduce((acc, [key, value]) => {
        const index = Object.keys(FEATURE_RANGES).indexOf(key);
        acc[index] = typeof value === "string" ? Number.parseInt(value, 10) : value;
        return acc;
      }, {} as Record<number, number>);

      // Convert to array in the correct order
      const inputArray = Object.keys(modelInput)
        .sort((a, b) => Number(a) - Number(b))
        .map(key => modelInput[Number(key)]);
        ...
        <div className="space-y-2">
          <Label htmlFor={name} className="flex items-center gap-1">
            ...
            <span 
              ...
              title={FEATURE_DESCRIPTIONS[name]}
            >
              (?)
            </span>
          </Label>
    ...
    <div className="container mx-auto px-4 py-12">
      <div className="max-w-4xl mx-auto">
        ...
        <Alert className="mb-6 bg-gray-100 border-gray-200">
          ...
          <AlertTitle className="text-gray-800">Medical Disclaimer</AlertTitle>
          <AlertDescription className="text-gray-700">
            This is an AI trained model for educational purposes only. Results should not replace professional medical advice.
          ...
          <Card>
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <Button type="submit" className="w-full bg-black hover:bg-opacity-90" disabled={isLoading}>
